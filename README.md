...
<img src="./assets/RGB, CMYKの分解.png" width="750px">
...
<img src="./assets/YCbCrの分解.png" width= "600px">
...
<img src="./assets/周波数解析.png" width="800px">
...
2次元に周波数解析を拡張するということは入力も2次元になるのは当然ですが出力の周波数スペクトルに関しても2次元になります。具体的なデータの構造ですが解析される周波数の波も2次元的になり縦、横の周波数が直行した周波数スペクトルのマトリクスとなります。
...
こちらのアルゴリズムの概要を簡単に説明すると前処理、後処理を行うことでN個の要素を持つ離散コサイン変換を2つの1/2個の要素2つの離散コサイン変換に分解する事ができます。この処理を再帰的に行うことで O (N log N) のオーダーで計算量を抑えることが出来ます。こちらのアルゴリズムですが1次元用なので2次元用に拡張する必要があり、拡張すると最終的には O (N^2 log N) のオーダーで計算量が必要になります。結果としては O (N^4 / (N^2 log N)) となり論理値としては概ね20倍ほど高速化を行うことが出来ると言うことになります。

余談にはなりますがサンプルプログラムの方では上記の論文の数式に対し数学的ではないアルゴリズム方面で冗長な処理を排除する余地が多分にあり結果としては高速化なしの実装と比べ40倍ほどの高速化を達成しています。
...
人の目は高周波数の情報には鈍感という特性があるので高周波数領域には特に大きな数値を設定し絶対値を小さくします。
...
```JavaScript
requantizedSample[v][u] = quantized_sample[v][u] * quantizationTable[v][u];
```

### 差分直流変換

ユニットの周波数スペクトル中の縦方向にも横方向にも完全に直流の成分に対する処理となります。こちらの処理としては左上から右に向かって1行ずつユニット毎にデータを出力していくのですが直流成分は前回、出力した直流成分との差分を取り、その値をデータとして出力を行います。

こちらの処理を行う理由は隣り合うユニットの直流成分同士は差分が小さくこの処理を行うことで出力の数値の絶対値を小さく出来、エントロピー符号化においてデータの圧縮率を上げる狙いがあります。
