...
まずは8×8の2次元配列の各ユニットに対しどのような順番でデータを出力するのかを解説します。

データを順次出力しレンダリングを行う関係上、低い周波数のデータから順番に出力するために図のようにジグザグにデータの転送を行います。これをJPEGの仕様ではジグザグシーケンスと読んでいます。

またこのジグザグシーケンスですが高周波数領域のデータを切り捨てる関係上、後半に行けば行くほど0が連続したデータが固まる特性上、エントロピー符号化による圧縮率が高くなります。
